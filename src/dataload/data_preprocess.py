...
def prepare_entity_sentiment(cfg, mode='train'):
    """
    处理实体的情感向量，将senti_entities.tsv中的数据处理成二进制格式
    Args:
        cfg: 配置
    ...
    # 检查是否已经处理过
    target_path = Path(data_dir[mode]) / "entity_sentiment.bin"
    if target_path.exists() and not cfg.reprocess:
        print(f"[{mode}] Entity sentiment vectors already exist!")
        return
    ...
    # 读取情感向量文件
    senti_file = Path(data_dir[mode]) / "senti_entities.tsv"
    if not senti_file.exists():
        print(f"[{mode}] Warning: senti_entities.tsv not found!")
        return
        
    # 初始化情感向量字典 {entity_id: sentiment_vector}
    entity_sentiment = {}
    default_sentiment = [0.0, 0.0, 0.0]  # 默认情感向量
    
    # 读取并解析情感向量
    with open(senti_file, 'r', encoding='utf-8') as f:
        next(f)  # 跳过标题行
        for line in tqdm(f, desc=f"[{mode}] Processing entity sentiment"):
            parts = line.strip().split('\t')
            if len(parts) != 3:
                continue
                
            news_id, num_entities, sentiment_data = parts
            try:
                sentiment_dict = json.loads(sentiment_data)
                
                # 处理每个实体的情感向量
                for entity_id, entity_info in sentiment_dict.items():
                    if entity_id in entity_dict:
                        # entity_info格式: [entity_name, sentiment_polarity, sentiment_vector]
                        entity_sentiment[entity_dict[entity_id]] = entity_info[2]
            except json.JSONDecodeError:
                print(f"Warning: Could not parse JSON in line: {line}")
                continue
    
    # 为没有情感向量的实体设置默认值
    for entity_id in range(1, len(entity_dict) + 1):
        if entity_id not in entity_sentiment:
            entity_sentiment[entity_id] = default_sentiment
    
    # 保存处理后的情感向量
    pickle.dump(entity_sentiment, open(target_path, "wb"))
    print(f"[{mode}] Finished processing entity sentiment vectors, saved to {target_path}")
    print(f"[{mode}] Total entities: {len(entity_dict)}, Entities with sentiment: {len(entity_sentiment)}")
...
def prepare_preprocessed_data(cfg):
    ...
    # 添加情感向量处理
    prepare_entity_sentiment(cfg, 'train')
    prepare_entity_sentiment(cfg, 'val')
    prepare_entity_sentiment(cfg, 'test')
    ...
    if os.name == 'nt':  # Windows 环境
        ...
        print("type " + f"{train_entity_emb_path} {val_entity_emb_path}" + f" > {val_combined_path}")
        print("type " + f"{train_entity_emb_path} {test_entity_emb_path}" + f" > {test_combined_path}")
        os.system("type " + f"{train_entity_emb_path} {val_entity_emb_path}" + f" > {val_combined_path}")
        os.system("type " + f"{train_entity_emb_path} {test_entity_emb_path}" + f" > {test_combined_path}")
    else:  # Linux/Unix 环境
